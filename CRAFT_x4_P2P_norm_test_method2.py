...
import network.CRAFT.craft_arch as craft
from network.CRAFT.craft_arch import CRAFT
...
from method2.uniontrain.method2_union_net_val import CRAFT_P2P_union2_val
...
kwards_x2 = {'type': CRAFT,'upscale': 2,'in_chans': 3,'img_size': 64,'window_size': 16,'img_range': 1.,'depths': [2, 2, 2, 2],
              'embed_dim': 48,'num_heads': [6, 6, 6, 6],'mlp_ratio': 2,'resi_connection': '1conv'}
kwards_x4 = {'type': CRAFT,'upscale': 4,'in_chans': 3,'img_size': 64,'window_size': 16,'img_range': 1.,'depths': [2, 2, 2, 2],
              'embed_dim': 48,'num_heads': [6, 6, 6, 6],'mlp_ratio': 2,'resi_connection': '1conv'}
kwards_x8 = {'type': CRAFT,'upscale': 8,'in_chans': 3,'img_size': 64,'window_size': 16,'img_range': 1.,'depths': [2, 2, 2, 2],
              'embed_dim': 48,'num_heads': [6, 6, 6, 6],'mlp_ratio': 2,'resi_connection': '1conv'}

# lpips_model = lpips.LPIPS(net="alex").cuda()

SRnet2 = craft.LR_SR_x2(kwards={'type': CRAFT, 'upscale': 2, 'in_chans': 3, 'img_size': 64, 'window_size': 16, 'img_range': 1.,
                     'depths': [2, 2, 2, 2],'embed_dim': 48, 'num_heads': [6, 6, 6, 6], 'mlp_ratio': 2, 'resi_connection': '1conv'}).cuda()
chpoint = torch.load("D:/code/Python/project/mechine Learning/block super resolution/multi scales SR/SRNet/CRAFT_P2P_quant_x2_norm.pt")
SRnet2.load_state_dict(chpoint['net'])
SRnet2.eval()

SRnet4 = craft.LR_SR_x4(kwards={'type': CRAFT, 'upscale': 4, 'in_chans': 3, 'img_size': 64, 'window_size': 16, 'img_range': 1.,
                    'depths': [2, 2, 2, 2],'embed_dim': 48, 'num_heads': [6, 6, 6, 6], 'mlp_ratio': 2, 'resi_connection': '1conv'}).cuda()
chpoint = torch.load("D:/code/Python/project/mechine Learning/block super resolution/multi scales SR/SRNet/CRAFT_P2P_quant_x4_norm.pt")
SRnet4.load_state_dict(chpoint['net'])
SRnet4.eval()

SRnet8 = craft.LR_SR_x8(kwards={'type': CRAFT, 'upscale': 8, 'in_chans': 3, 'img_size': 64, 'window_size': 16, 'img_range': 1.,
                     'depths': [2, 2, 2, 2],'embed_dim': 48, 'num_heads': [6, 6, 6, 6], 'mlp_ratio': 2, 'resi_connection': '1conv'}).cuda()
chpoint = torch.load("D:/code/Python/project/mechine Learning/block super resolution/multi scales SR/SRNet/CRAFT_P2P_quant_x8_norm.pt")
...
union=CRAFT_P2P_union2_val().cuda()
chpoint=torch.load('D:\code\Python\project\mechine Learning/block_rescaling_32\method2/uniontrain/net/CRAFT_P2P_union2/CRAFT_P2P_union2.pt')
...
#采样率分配
    ...
    # block_x2, block_x4, block_x8 = sampling_rate_allocate3_ablation_study(img_BICUBIC_x4, img1)
    print(len(block_x2),len(block_x4),len(block_x8))
    ...
    # cv2.imwrite('F:/BMDS_output/contrast/' + "CRAFT_P2P_MAS_"+imgname, (i1 * 255)[int(index / rows) * 16:int(index / rows) * 16 + 256, index % rows * 16:index % rows * 16 + 256,:])


    # cv2.imwrite('F:\BMDS_output\MultiLevelBMDSR\CRAFT_P2P_test4k/' + imgname, i1*255)
    i1 = cv2.cvtColor(i1, cv2.COLOR_RGB2BGR)
