...
import lpips
import time
from select_rate import sampling_rate_allocate3
import sys
...
from method2.uniontrain.method2_union_net_val import OmniSR_union2_val
...
SRnet4 = LR_SR_x4(kwards={'upsampling': 4, 'res_num': 5, 'block_num': 1, 'bias': True, 'block_script_name': 'OSA',
                    'block_class_name': 'OSA_Block', 'window_size': 8,
                    'pe': True,
                    'ffn_bias': True}).cuda()
chpoint = torch.load("D:/code/Python/project/mechine Learning/block_rescaling_32/SRNet/OmniSR_x4_norm.pth")
print(chpoint)
SRnet4.load_state_dict(chpoint)
SRnet4.eval()
for name, param in SRnet4.named_parameters():
    param.requires_grad = False

SRnet2 = LR_SR_x2(kwards={'upsampling': 2, 'res_num': 5, 'block_num': 1, 'bias': True, 'block_script_name': 'OSA',
                    'block_class_name': 'OSA_Block', 'window_size': 8,
                    'pe': True,
                    'ffn_bias': True}).cuda()
chpoint = torch.load("D:/code/Python/project/mechine Learning/block_rescaling_32/SRNet/OmniSR_x2_norm.pth")
SRnet2.load_state_dict(chpoint)
SRnet2.eval()
for name, param in SRnet2.named_parameters():
    param.requires_grad = False

SRnet8 = LR_SR_x8(kwards={'upsampling': 8, 'res_num': 5, 'block_num': 1, 'bias': True, 'block_script_name': 'OSA',
                    'block_class_name': 'OSA_Block', 'window_size': 8,
                    'pe': True,
                    'ffn_bias': True}).cuda()
chpoint = torch.load("D:/code/Python/project/mechine Learning/block_rescaling_32/SRNet/OmniSR_x8_norm.pth")
SRnet8.load_state_dict(chpoint)
SRnet8.eval()
for name, param in SRnet8.named_parameters():
    param.requires_grad = False

union=OmniSR_union2_val().cuda()
chpoint=torch.load('D:\code\Python\project\mechine Learning/block_rescaling_32\method2/uniontrain/net/Omni_union2/Omni_union2.pt')
...
# photo_path = 'D:\code\Python\project\mechine Learning\Data/Urban100/HR'
# photo_path='D:\code\Python\project\mechine Learning\Data\BSD68\BSD68'
...
#采样率分配
    ...
    # l2=transforms.ToPILImage()(temp_x4[0])
    # l2.show()
    ...
    # index=block_x2[3]
    index=35
    # cv2.imwrite('F:\BMDS_output\MultiLevelBMDSR\OmniSR_test4k/' + imgname, i1*255)
    i1 = cv2.cvtColor(i1, cv2.COLOR_RGB2BGR)
