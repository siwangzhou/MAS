...
import torch
from network.ops.edsr.edsr import EDSR,LR_SR_x8,LR_SR_x4,LR_SR_x2
from network.ops.OSAG import OSAG
# from method2.uniontrain.method2_union_net import OmniSR_P2P_union2
from method2.uniontrain.method2_union_net_val import *
...
SRnet2 = edsr.LR_SR_x2().cuda()
chpoint= torch.load("D:\code\Python\project\mechine Learning/block_rescaling_32\SRNet/EDSR_P2P_quant_x2_norm.pt")
SRnet2.load_state_dict(chpoint['net'])
SRnet2.eval()

SRnet4 = edsr.LR_SR_x4().cuda()
...
SRnet8 = edsr.LR_SR_x8().cuda()
chpoint= torch.load("D:\code\Python\project\mechine Learning/block_rescaling_32\SRNet/EDSR_P2P_quant_x8_norm.pt")
...
union=EDSR_P2P_union2_val().cuda()
chpoint=torch.load('D:\code\Python\project\mechine Learning/block_rescaling_32\method2/uniontrain/net/EDSR_P2P_union2/EDSR_P2P_union2.pt')
...
for imgname in tqdm(test_list):
    ...
    # flops_temp1, params = profile(SRnet4.layer1, inputs=(img1.cuda(),))
    # flops_temp2, params = profile(SRnet2.layer1, inputs=(img1.cuda(),))
    # flops_temp3, params = profile(SRnet8.layer1, inputs=(img1.cuda(),))
    # flops_sum+=flops_temp1+flops_temp2+flops_temp3
    # print(flops_temp)

    torch.cuda.empty_cache()
    t1 = time.time()
    # img_LR4_noblock,temp_x4=checkpoint(SRnet4, (img_BICUBIC_x4.cuda()-0.5)*2, use_reentrant=True)
...
#采样率分配
    ...
    # print(block_x4,sep='\n')
    # t1=time.time()
    # block_x2, block_x4, block_x8 = sampling_rate_allocate3((temp_x4/2+0.5).cpu(), img1)
    ...
    #     # continue
        torch.cuda.empty_cache()
        _, source = checkpoint(SRnet4, (img1.cuda()-0.5)*2, use_reentrant=True)
        source=source/ 2 + 0.5
    ...
    # cv2.imwrite('F:/BMDS_output/contrast/' + "EDSR_P2P_MAS_"+imgname, (i1 * 255)[int(index / rows) * 16:int(index / rows) * 16 + 256, index % rows * 16:index % rows * 16 + 256,:])

    # cv2.imwrite('F:\BMDS_output\MultiLevelBMDSR\EDSR_P2P_test4k/' + imgname, i1*255)
    i1 = cv2.cvtColor(i1, cv2.COLOR_RGB2BGR)
...
print(block_imgnums)
print('flops:',flops_sum/block_imgnums/10**9)
