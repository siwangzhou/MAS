...
from method2.uniontrain.method2_union_net_val import CRAFT_union2_val
...
kwards_x4 = {'type': CRAFT, 'upscale': 4, 'in_chans': 3, 'img_size': 64, 'window_size': 16, 'img_range': 1.,
                     'depths': [2, 2, 2, 2],
                     'embed_dim': 48, 'num_heads': [6, 6, 6, 6], 'mlp_ratio': 2, 'resi_connection': '1conv'}
SRnet4 = CRAFT(**kwards_x4).cuda()
chpoint = torch.load("D:\code\Python\project\mechine Learning/block_rescaling_32\SRNet/craft_x4.pt")
SRnet4.load_state_dict(chpoint)

for name, param in SRnet4.named_parameters():
    param.requires_grad = False

union=CRAFT_union2_val().cuda()
chpoint=torch.load('D:\code\Python\project\mechine Learning/block_rescaling_32\method2/uniontrain/net/CRAFT_union2/CRAFT_union2.pt')
...
#采样率分配
    ...
    # index=block_x2[3]
    index=35
    # cv2.imwrite('F:\BMDS_output\MultiLevelBMDSR\CRAFT_test4k/' + imgname, i1*255)
    i1 = cv2.cvtColor(i1, cv2.COLOR_RGB2BGR)
