...
#采样率分配
    ...
    img_SR2 = checkpoint(SRnet2.layer1, HR2 * 2 - 1)
    img_LR2 = checkpoint(SRnet2.layer2, img_SR2)
    ...
    for i in block_x4:
        ...
        temp2:i % rows * patch_size + patch_size] = img1[:, :, temp1:int(
            i / rows) * patch_size + patch_size, temp2:i % rows * patch_size + patch_size]
    img_SR4 = checkpoint(SRnet4.layer1, HR4 * 2 - 1)
    img_LR4 = checkpoint(SRnet4.layer2, img_SR4)
    ...
    for i in block_x8:
        ...
        temp2:i % rows * patch_size + patch_size] = img1[:, :, int(i / rows) * patch_size:int(
            i / rows) * patch_size + patch_size, temp2:i % rows * patch_size + patch_size]
    img_SR8 = checkpoint(SRnet8.layer1, HR8*2-1)
    img_LR8 = checkpoint(SRnet8.layer2, img_SR8)
    ...
    if len(block_x2) >= 0:
        ...
        torch.cuda.empty_cache()
        for i in block_x2:
            LR2[:, :, int(i / rows) * patch_size_x2:int(i / rows) * patch_size_x2 + patch_size_x2,
            i % rows * patch_size_x2:i % rows * patch_size_x2 + patch_size_x2] = img_LR2[:, :,
        ...
        img_SR2= checkpoint(SRnet2.layer3, LR2)
        img_SR4 = checkpoint(SRnet4.layer3, LR4)
        img_SR8 = checkpoint(SRnet8.layer3, LR8)
        print(img_SR2.shape,LR2.shape)
        # 分块超分
        # x2
        # LR2 = torch.zeros_like(img_LR2)+0.5
        # LR2 = img_LR2
        torch.cuda.empty_cache()
        for i in block_x2:
            source[:, :, int(i / rows) * patch_size:int(i / rows) * patch_size + patch_size, i % rows * patch_size:i % rows * patch_size + patch_size]=img_SR2[:, :, int(i / rows) * patch_size:int(i / rows) * patch_size + patch_size, i % rows * patch_size:i % rows * patch_size + patch_size]
        ...
        torch.cuda.empty_cache()
        for i in block_x4:
            source[:, :, int(i / rows) * patch_size:int(i / rows) * patch_size + patch_size, i % rows * patch_size:i % rows * patch_size + patch_size]=img_SR4[:, :, int(i / rows) * patch_size:int(i / rows) * patch_size + patch_size, i % rows * patch_size:i % rows * patch_size + patch_size]
        ...
        torch.cuda.empty_cache()
        for i in block_x8:
            source[:, :, int(i / rows) * patch_size:int(i / rows) * patch_size + patch_size, i % rows * patch_size:i % rows * patch_size + patch_size]=img_SR8[:, :, int(i / rows) * patch_size:int(i / rows) * patch_size + patch_size, i % rows * patch_size:i % rows * patch_size + patch_size]
